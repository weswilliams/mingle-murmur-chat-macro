namespace :macro do |ns|
  ...
  task :clean_deploy do
    ...
    FileUtils.rm_rf(File.join(ENV['MINGLE_LOCATION'], 'vendor', 'plugins', "murmur_chat"))
  end

  task :deploy do
    macro_folder = File.expand_path(".")
    mingle_plugins_folder = File.join(ENV['MINGLE_LOCATION'], 'vendor', 'plugins', "murmur_chat")
